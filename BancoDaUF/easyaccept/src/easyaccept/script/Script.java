 ...
 * The returned value can then be tested (using an internal command such as <code>expect</code>).
...
public class Script implements MultiFileReaderObserver, Runnable {
	...
	/**
	 * The ScriptResultManager is responsible for to storage info about results of a test script.
	 */
	private ScriptResultsManager scriptResultManager;
	 ...
	 */
	public Script(String fileName, Facade facade, Variables variables)
		...
		this.facade = facade;
		this.scriptResultManager = new ScriptResultsManager(fileName);
	 ...
	 *             if the script file cannot be found.
	 */
	public Script(String fileName, Facade facade) throws EasyAcceptException,
	...
	Result executeCommand(ParsedLine parsedLine) throws QuitSignalException {
		if (parsedLine != null) {
			...
			return new ResultImpl(parsedLine.getCommandString(stringDelimiter),
					result, cause);
		} else {
			return null;
		}
	 ...
	 * @throws Exception 
	 */
	private Object execute(ParsedLine parsedLine)throws Exception {
		assert parsedLine.numberOfParameters() > 0;
	...
	public int numberOfErrors() {
		return scriptResultManager.getNumberOfErrors();
	}
	...
	private void execute() throws EasyAcceptException, IOException,
		...
		while ((oneResult = getAndExecuteCommand()) != null) {
			this.scriptResultManager.addResult(oneResult, plr.getLineNumber());
	...
	public String allErrorMessages() {
		...
		Iterator<Result> it = scriptResultManager.getResults().values().iterator();
		while (it.hasNext()) {
	...
	public int numberOfTests() {
		return scriptResultManager.getResults().size();
	}
...
//			System.out.println("\n" + e.getMessage());
	 ...
	 * @return ScriptResultManager An object that contains the result of each script line.
	 */
	public ScriptResultsManager getResultManager() {
		return this.scriptResultManager;
	}
