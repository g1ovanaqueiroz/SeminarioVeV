...
/**
 * This is the EasyAccept facade. It enables its users to request tests execution 
 * and to get the tests results. The Facade makes available many information about the executed tests.
 * 
...
public class EasyAcceptFacade {
	 ...
	 * 
	 * @param facade
	 *            	the facade object, used to execute application commands and to obtain its results.
	 * @param testFileName
	...
	private boolean runAcceptanceTest(Facade facade, String file, Variables variables) throws IOException, FileNotFoundException,
		...
		resultsHandler.addResult(scriptResultsManager.getFile(), scriptResultsManager);
		return true;
	}
	...
	public void executeTests() {
		try {
			for (int i = 0; i < files.size(); i++) 
					runAcceptanceTest(this.facadeAdapter, files.get(i), new VariablesImpl());
		} catch (Exception e) {}
	}
	 ...
	 * @param file 
	 * 				the name of the file that contains the executed acceptance tests.
	 * @return The errors of the executed test script (if they occur). 
	 * 		   The empty string "" is returned if no errors have been occurred.
			   null is returned if there is no test file with the specified name.
	 */
